...
<div class="tabs">
	...
	<div class="tabsContent" >
		<div>
			<form method="post" action="__URL__/setApp" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone)">
				...
				<div class="pageFormContent" layoutH="100">
					<?php if(is_array($appList)): $i = 0; $__LIST__ = $appList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><div class="unit">			
							<input <?php echo in_array($key, $groupAppList) ? "checked" : "" ?> type="checkbox" name="groupAppId[]" value="<?php echo ($key); ?>"/><?php echo ($item); ?>
		...
		<div>
			<!-- <?php if(empty($_GET['appId'])): ?><form method="post" action="__URL__/setModule" class="pageForm required-validate" onsubmit="return validateCallback(this, dialogAjaxDone)">
		<input type="hidden" name="groupId" VALUE="<?php echo ($_GET['groupId']); ?>" />
		<div class="pageFormContent" layoutH="100" layoutType="dialog">
			<div class="unit">
				<label>Applicaion:</label>
				<select name="appId" onchange="selectApp_module('<?php echo ($_GET['groupId']); ?>', this.value)">
					<option value="">Choose</option>
					<?php if(is_array($appList)): $i = 0; $__LIST__ = $appList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php echo in_array($key, $selectAppId) ? "selected" : "" ?>><?php echo ($item); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
				</select>
			</div>
			
			<div id="moduleSelectBox">
			<?php if(is_array($moduleList)): $i = 0; $__LIST__ = $moduleList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><div class="unit">
				<input <?php echo in_array($key, $groupModuleList) ? "checked" : "" ?> type="checkbox" name="groupModuleId[]" value="<?php echo ($key); ?>"/><?php echo ($item); ?>
			</div><?php endforeach; endif; else: echo "" ;endif; ?>
			</div>
		</div>
		<div class="formBar">
...
<?php else: ?>
 ...
 -->
		...
		<div>
			<!-- <?php if(!empty($_GET['appId']) and !empty($_GET['moduleId'])): if(is_array($actionList)): $i = 0; $__LIST__ = $actionList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><div class="unit">
...
<?php elseif(!empty($_GET['appId'])): ?>
	...
	<select name="moduleId" onchange="selectModule_action('#setActionAction')">
		<option value="">Choose</option>
		<?php if(is_array($moduleList)): $i = 0; $__LIST__ = $moduleList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><option value="<?php echo ($key); ?>" <?php echo in_array($key, $selectModuleId) ? "selected" : "" ?>><?php echo ($item); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
	</select>
</div>

<div id="actionSelectBox">
	<?php if(is_array($actionList)): $i = 0; $__LIST__ = $actionList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><div class="unit">
		<input <?php echo in_array($key, $groupActionList) ? "checked" : "" ?> type="checkbox" name="groupActionId[]" value="<?php echo ($key); ?>"/><?php echo ($item); ?>
	</div><?php endforeach; endif; else: echo "" ;endif; ?>
</div>
